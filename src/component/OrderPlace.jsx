...
const OrderPlace = () => {
          ...
          <p className="mb-0 text-muted">
            ...
            <small>
              ...
              {(
                ...
                cloth.clothsId.price * cloth.clothsId.discount * 0.01
              ).toFixed(2)}
            </small>
          </p>
        </div>
  ...
  const { address, addressStatus, addressError } = useSelector(
  ...
  const allAddress = address?.map((address) =>
    Object.values(address).slice(2, 10).join(", ")
  );
  ...
  useEffect(() => {
    if (orderState) {
      const cloths = orderState.cloths.map((cloth) => ({
        clothsId: cloth.clothsId._id,
  ...
  const loadingCondition =
    userStatus === "userinfo/Loading" ||
    addressStatus === "userAddress/Loading";

  const renderCondition =
    orderState &&
    userStatus === "userinfo/success" &&
    addressStatus === "userAddress/success";
  ...
  const userInfo = (
    <>
      ...
      <div>
        {allAddress &&
          allAddress.map((address) => {
            return (
              <div className="form-check" key={address}>
                <input
                  className="form-check-input"
                  checked={selectedAdress === address}
                  type="radio"
                  name="address"
                  id={address}
                  value={address}
      ...
      {error && !selectedAdress && (
        <p className="text-danger">Please Add All Information</p>
      )}
      <div className="d-flex align-items-center mt-3">
        ...
        {allAddress.length < 1 && (
          <Link className="fs-6 btn btn-primary" to={`/user/profile/address`}>
            Add New Address
          </Link>
        )}
      </div>
  ...
  return (
    <>
      {orderState && orderStaus !== "addOrder/Success" && (
        <div className="container py-4">
          <h1>Order Details</h1>

          <div>
            <ul>{orderClothsComp}</ul>
            ...
            <>
              ...
              <hr />
              <p>
                <span>Pirce ({orderState.cloths.length} Items) : </span>
                <span> &#8377;{orderState.priceDetails.totalPrice}</span>
              </p>
              <p>
                Discount:{" "}
                <span style={{ color: "green" }}>
                  &minus;&#8377;{orderState.priceDetails.discount}
                </span>
              </p>
              <p>
                Discounted Price:{" "}
                <span style={{ color: "green" }}>
                  &#8377;{orderState.priceDetails.discountedPrice}
                </span>
              </p>
              <p>
                Delivery Charges:
                {orderState.priceDetails.deliveryCharges === 0 ? (
                  <>
                    {" "}
                    <span className="fs-6 text-decoration-line-through fw-light ">
                      {" "}
                      &#8377;40
                    </span>
              ...
              <p style={{ color: "green" }}>
                You will save &#8377;{orderState.priceDetails.discount} on this
                order
              </p>
            </>
          </div>
          ...
          <div>{renderCondition && userInfo}</div>
        </div>
      )}
